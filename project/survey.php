        ...
        if ($results) {
            ...
            foreach ($results as $index => $group) {
                foreach ($group as $details) {
                    if($details["visibility"] == 0 && get_user_id() != $details["user_id"] && !has_role("Admin")) {
                        flash("You don't have permission to access this page");
                        die(header("Location: public_surveys.php"));
                    }
                        ...
                        <div>
                            <?php foreach ($question["answers"] as $answer): ?>
                                ...
                                <div class="form-check">
                                    <input requried class="form-check-input" type="radio" autocomplete="off" name="<?php safer_echo($index); ?>" id="option-<?php echo $eleId; ?>" value="<?php safer_echo($answer["answerId"]); ?>">
